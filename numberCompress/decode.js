...
const dict = s.split('').reduce((dict, char, index) => {
    dict[char] = index
    return dict
}, {})
const base = s.length
...
function decode(string) {
    ...
    for (let i = 0; i < input.length; i++) {
        number += dict[input[i]] * Math.pow(base, i)
    }
    return number
}

let csvfile = process.argv[2] || 'encode.txt'

console.log('Reading', csvfile, '...')

let content = fs.readFileSync(csvfile, 'utf-8')
let maxlength = parseInt(content[0])
let match = new RegExp(`.{1,${maxlength}}`, 'g')
let arr = content.slice(1).match(match);
let numbers = arr.map(decode)
fs.writeFileSync('decode.csv', numbers.join('\n'), 'utf-8')
